...
while (!arrayBombe.includes(numeroUtente) && i-1 < numeriPossibili) {
  ...
  while (numeroUtente < 1 || numeroUtente > limiteRange || isNaN(numeroUtente) || arrayUtente.includes(numeroUtente)) {
    if (numeroUtente < 1 || numeroUtente > limiteRange || isNaN(numeroUtente)) {
      numeroUtente = parseInt(prompt('Valore immesso errato! Inserire un numero da 1 a ' + limiteRange + ' dove pensi non ci sia una bomba'));
    } else {
      numeroUtente = parseInt(prompt('Valore immesso già scelto precedentemente! Inserire un numero da 1 a ' + limiteRange + ' dove pensi non ci sia una bomba'));
    }
  }
